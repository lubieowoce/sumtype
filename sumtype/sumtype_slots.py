...
def is_type_or_typing_type(t) -> bool:
	return type(t) is type or type(t).__module__ == 'typing'
...
def typed_spec_errors(typename: str, variant_specs: List[  Tuple[str, List[Tuple[str, type]] ]  ]) -> List[str]:
	...
	for (variant, fields_and_types) in variant_specs:
		...
		for (field, type_) in fields_and_types:
			...
			assert is_type_or_typing_type(type_), "{!r} ({!r})".format(type_, type(type_)) 
...
def type_literal(t) -> str:
	assert is_type_or_typing_type(t), repr(t)
...
def sumtype(
	...
	errors = typed_spec_errors(name, variant_specs)
	if errors:
		raise ValueError('\n'.join(errors))
	...
	class Class:
		...
		@classmethod
		def _from_dict(cls, dct: dict) -> name:
			dct_ = dct.copy()
		...
		def _variant_reprs(cls) -> Iterator[str]:
			return (
				"{var}({fields})".format(var=variant, fields=str.join(', ', (field+': '+type_literal(type_) for (field, type_) in cls._variant_specs[variant])))
	...
	def mk_def_replace(typename, immutable, variant_ids, variants, variant_id_fields, _set_attr):
		with_fields    = [
			(id_, variant, fields)
			for (id_, variant, fields) in zip(variant_ids, variants, variant_id_fields)
			if fields
	...
	for (id_, variant) in zip(Class._variant_ids, Class._variants):
		...
		constructor.__annotations__.update({field: type_ for (field, type_) in fields_and_types if type_ is not Any and type_ is not object})
