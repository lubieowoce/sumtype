...
def sumtype(
	...
	class Class:
		...
		def raise_direct_init_forbidden(self) -> NoReturn:
			...
			raise TypeError(
				('Cannot instantiate {type!r} directly - '+
				 'use one of the variant constructors: {vars}') \
					.format(
			...
			if any(attr in fields for fields in cls._variant_fields.values()):
				...
				ex = AttributeError(
					("Incorrect '{type}' variant: "+
					 "Field '{attr}' not declared in variant '{var}'. Variants: {vars}") \
						.format(
				...
				ex = AttributeError(
					("Unknown attribute: "+
					 "Field '{attr}' not declared in any variant of '{type}': {vars}") \
						.format(
		...
		def _raise_delattr_forbidden(self, attr: str) -> NoReturn:
			...
			raise TypeError(
				"Cannot modify '{type}' values." \
					.format(type=self.__class__.__qualname__)
			)
					...
					res += "({fields})".format(
						fields=str.join(
							...
							(field+': '+type_literal(type_)
							 for (field, type_) in variant_spec)
						)
					)
				yield res
			...
			warn(
				RuntimeWarning(
					("`untyped_sumtype` can't access the caller's module name - "+
					 "pickle will likely have trouble unpickling {!r} objects") \
						.format(typename)
				)
			)
